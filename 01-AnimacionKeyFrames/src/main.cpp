...
GLuint textureCespedID, textureWallID, textureWindowID, textureHighwayID, textureWaterID, textureSpongeID; //, textureLandingPadID;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	boxEjercicio.init();
	boxEjercicio.setShader(&shaderMulLightingTex);
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderMulLighting.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		// Setter de las matrices para nuevo che
		...
		boxEjercicio.setPosition(glm::vec3(0.0, 1.0, -5.0));
		boxEjercicio.render();
